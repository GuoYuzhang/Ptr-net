...
class PointerNet:
    ...
    def build_graph(self, mode):
        ...
        logits, sample_ids = self._build_decoder(enc_outputs, enc_state)
        loss = self._compute_loss(logits)
        return logits, loss, sample_ids
    ...
    def _compute_loss(self,logits):
        ...
        crossent = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=trgt_out, logits=logits)
        trgt_weight = tf.sequence_mask(self.trgt_length, max_time, dtype=logits.dtype)
        loss = tf.reduce_sum(crossent * trgt_weight) / tf.to_float(self.batch_size)
        return loss
